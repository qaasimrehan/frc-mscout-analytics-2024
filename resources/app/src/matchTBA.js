class matchTBA {
    constructor() {
        ...
        this.tbaDBLm = new Datastore({filename: require('electron').app.getAppPath() + "/tbadb_lm.db", autoload: true});
        this.tbaDBTeam = new Datastore({
            filename: require('electron').app.getAppPath() + "/tbadb_team.db",
            autoload: true
        });
    }
        ...
        if (matchObj.matchType == "T" || matchObj.matchType == "PF" || matchObj.matchType == "PM") {
            ...
            xhttp.onreadystatechange = () => {
                if (xhttp.readyState == 4 && (xhttp.status == 200 || xhttp.status == 304)) {
                    ...
                    if (tar != null) {
                        var compLvl = "";
                        if (matchObj.matchType == "Q") {
                            compLvl = "qm";
                        }
                        if (matchObj.matchType == "QF") {
                            compLvl = "qf";
                        }
                        if (matchObj.matchType == "SF") {
                            compLvl = "sf";
                        }
                        if (matchObj.matchType == "F") {
                            compLvl = "f";
                        }
                        ...
                        xhttp2.onreadystatechange = () => {
                            if (xhttp2.readyState == 4 && xhttp2.status == 200) {
                                ...
                                if (contained != null) {
                                    ...
                                    res.lastModified = {
                                        event: xhttp.getResponseHeader("Last-Modified"),
                                        match: xhttp2.getResponseHeader("Last-Modified")
                                    };
                                    if (matchObj.tbaData != undefined) {
                        ...
                        if (xhttp.status == 200) {
                            ...
                            if (res.length > 0) {
                                ...
                                if (eventEmitter != null) {
                                    eventEmitter.sender.send("query-team-sync-track", {
                                        position: res.length,
                                        total: res.length,
                                        page: index
                                    })
                            ...
                            if (eventEmitter != null) {
                                eventEmitter.sender.send("query-team-sync-track", {position: -1, total: 0, page: index})
                            }
