class matchLoader {
    ...
    getScoreInterval(index) {
        ...
        var objectTypeTmp = "";
        var failedTmp = true;
        if (this.data[index].key == "g" || this.data[index].key == "h") {
            scoreTypeTmp = "cargoShip";
            failedTmp = this.data[index].key == "h";
        }
        if (this.data[index].key == "i" || this.data[index].key == "j") {
            scoreTypeTmp = "rocket1";
            failedTmp = this.data[index].key == "j";
        }
        if (this.data[index].key == "k" || this.data[index].key == "l") {
            scoreTypeTmp = "rocket2";
            failedTmp = this.data[index].key == "l";
        }
        if (this.data[index].key == "m" || this.data[index].key == "n") {
            scoreTypeTmp = "rocket3";
            failedTmp = this.data[index].key == "n";
        }
        var stop = false;
        for (var i = index - 1; i >= 0 && !stop; i--) {
            ...
            if (this.data[i].key == "c" && objectTypeTmp == "hatch") {
                stop = true;
            }
            if (this.data[i].key == "e" && objectTypeTmp == "cargo") {
                stop = true;
            }
            if (this.data[i].key == "c" && !stop && objectTypeTmp == "cargo") {
                intervalWithFailTmp = this.data[index].value - this.data[i].value;
            }
            if (this.data[i].key == "e" && !stop && objectTypeTmp == "hatch") {
                intervalWithFailTmp = this.data[index].value - this.data[i].value;
            }
            if (this.data[i].key == "c" && intervalNoFailTmp == -1) {
                intervalNoFailTmp = this.data[index].value - this.data[i].value;
                intervalWithFailTmp = intervalNoFailTmp;
                objectTypeTmp = "cargo";
            }
            if (this.data[i].key == "e" && intervalNoFailTmp == -1) {
                intervalNoFailTmp = this.data[index].value - this.data[i].value;
                intervalWithFailTmp = intervalNoFailTmp;
                objectTypeTmp = "hatch";
            }
