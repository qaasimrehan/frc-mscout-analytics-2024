...
var init = () => {
    ...
    var str = "";
    if (metadata.matchType == "T") {
        str = "Test Match Number ";
    }
    if (metadata.matchType == "PF") {
        str = "Practice Field Match Number ";
    }
    if (metadata.matchType == "PM") {
        str = "Practice Match Number ";
    }
    if (metadata.matchType == "Q") {
        str = "Qualification Match Number ";
    }
    if (metadata.matchType == "QF") {
        str = "Quarterfinal Match Number ";
    }
    if (metadata.matchType == "SF") {
        str = "Semifinal Match Number ";
    }
    if (metadata.matchType == "F") {
        str = "Final Match Number ";
    }
    ...
    ipcRenderer.once("query-team-reply", (event, docs) => {
        ...
        if (team != undefined && team != null) {
            ...
            if (team.website != undefined && team.website != null && team.website.length < 45) {
                ...
                document.getElementById("teamUrl").onclick = () => {
                    require('electron').shell.openExternal(team.website);
                };
            } else {
            ...
            document.getElementById("tbaUrl").onclick = () => {
                require('electron').shell.openExternal("https://www.thebluealliance.com/team/" + team.team_number);
            };
        } else {
