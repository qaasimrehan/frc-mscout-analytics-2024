    ...
    ipcRenderer.once("query-team-reply", (event, docs) => {
        ...
        for (var i = 0; i < stats.availableTeams.length; i++) {
            ...
            for (var j = 0; j < tableData.length; j++) {
                if (tableData[j].team_number == stats.availableTeams[i].team) {
                    tableData[j].totalMatches = stats.availableTeams[i].totalMatches;
                    found = true;
                }
            }
            if (!found) {
                tableData.push({
                    team_number: stats.availableTeams[i].team,
                    totalMatches: stats.availableTeams[i].totalMatches,
                    nickname: "Team " + stats.availableTeams[i].team
                })
            }
        }
